...
describe('sorting and limiting with different query operations', async () => {
  ...
  const groupTracksByAlbum = () => {
    const grouped = tracks.sort(canonicalComparator).reduce(
      (acc, t) => {
    ...
    return ret.sort(canonicalComparator);
  };
  ...
  test.each([
    ...
    // TODO: test that join removes reply
    // TODO: test that source does not create new sources for:
    // - same order, desc/asc change, undefined order
    {
    ...
    // TODO(mlaw): removing things to cause us to come under the limit re-fills the window
