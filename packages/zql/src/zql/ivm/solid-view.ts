...
export class SolidView implements Output {
  ...
  // TODO: I'm not sure what should be the mechanism to update this store.
  // - use setRoot() and path syntax all the way through the tree?
  // - create a separate setRoot at each level of the tree? (I guess at EntryList would make most sense)
  // - use the mutable pattern and save the draft at end?
  // But this also brings up how transactions should work. How should we
  // prevent intermediate values from updating Solid components.
  // For example on initial sync there will be many values, but they flow through IVM
  // independently. We don't want to update the DOM until the end.
  // The React impl uses onTransactionCommit() to know that it's time to flush
  // a transaction, but how to actually do the pause/resume in solid?
  // There is batch() and mutableUpdate() but they are synchronous callback things.
  // this doesn't work with ivm because ivm is the source of the tx boudaries, not apps.
  // If there was startBatch() / endBatch() that would , but there's not.
  // I think we need "conditional signals"? Basically we need to disable rendering
  // a component (or ideally, just prevent sending updates) temporarily
  readonly #root: Entry;
